...
const StudentDetail = () => {
  ...
  const handleGetTeachHis = () => {
    httpClient.get('http://localhost:5000/class-student/' + id,)
      .then((res) => {
      ...
      <div id='modalPurchaseDetail'>
        <Card className="custom-card custom-card-listpage" title="Thông tin giáo viên" style={{ paddingBottom: 30 }}>
          <Row style={{ marginBottom: "7px" }}>
            <Col span={17}></Col>
            <Col span={6}><span style={{ fontSize: 12 }}>Ngày bắt đầu học tập: 07/07/2022</span></Col>
          </Row>
          ...
          <div className="custom-table-mail">
            <div className="table-row bd-bottom">
              <div className="__th bd-right">Lịch sử học tập</div>
              <div className='__td'>
                {
                  teachHistory?.map((item) => {
                    return (
                      <div className='__td_child'><span>Học kì: {item.semester.name}</span> <span style={{ width: 70, display: 'inline-block' }}></span> <span>Lớp: {item.class.name}</span> <span style={{ width: 70, display: 'inline-block' }}></span> <span>Tổng kết: {item.totalGrade/item.totalFactor}</span> <span style={{ width: 70, display: 'inline-block' }}></span> <span>Hạnh kiểm: {item.morality ? 'Chưa đánh giá' : item.morality}</span> </div>
                    )
                  })
                }
              </div>
